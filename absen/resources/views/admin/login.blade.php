...
<div class="container">
    ...
    <div class="row">&nbsp;</div>
    <div class="row">&nbsp;</div>
    <div class="row title text-color-primary m-2">Selamat Datang <br> Kembali Admin</div>
    <div class="row">&nbsp;</div>
    @if(Session::has('error'))
           <span class="error mb-2"> {{ Session::get('error') }}</span>
    @endif
    <form action="{{route('loginadmin.store')}}" method="post" class="form">
        @csrf
        ...
        <div class="row">
            ...
            <div class="col-3 text-right login-text-help">
                <a href="{{route('registeradmin.index')}}" class="text-decor-none">Daftar</a>
            </div>
        </div>
    </form>
